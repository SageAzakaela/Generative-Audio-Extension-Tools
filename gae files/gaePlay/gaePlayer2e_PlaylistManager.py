...
def generate_composition_in_thread(seed, generation_pattern, gae_file_path):
    ...
    # Extract the contents of the selected GAE file into the temporary directory
    with zipfile.ZipFile(gae_file_path, 'r') as gae_zip:
        gae_zip.extractall(temp_dir)
        print(f"Extracted contents of {gae_file_path} to {temp_dir}")
...
# Function to open the playlist manager window
def open_playlist_manager_window():
    playlist_manager_window = tk.Toplevel(app)
    playlist_manager_window.title("Playlist Manager")

    # Function to load a selected GAE file into the main player
    def load_gae_file(folder, file):
        # Set the GAE file entry to the selected file
        gae_file_entry.delete(0, tk.END)
        gae_file_entry.insert(0, join(folder, file))

    # Create and place a label for the folder selection
    folder_label = tk.Label(playlist_manager_window, text="Select a folder containing GAE files:")
    folder_label.pack()

    # Create and place a button to browse for the folder
    folder_button = tk.Button(playlist_manager_window, text="Browse", command=lambda: select_folder(playlist_manager_window))
    folder_button.pack()

    # Create a list to store GAE file buttons
    gae_file_buttons = []

    # Function to select a folder and display GAE file buttons
    def select_folder(window):
        folder_path = filedialog.askdirectory(title="Select Folder")
        if folder_path:
            # Get a list of GAE files in the selected folder
            gae_files = [f for f in listdir(folder_path) if isfile(join(folder_path, f)) and f.lower().endswith(".gae")]

            # Create buttons for each GAE file
            for gae_file in gae_files:
                button = tk.Button(window, text=gae_file, command=lambda file=gae_file: load_gae_file(folder_path, file))
                button.pack()
                gae_file_buttons.append(button)
            # Create and place a button to close the playlist manager window
            close_button = tk.Button(playlist_manager_window, text="Close", command=playlist_manager_window.destroy)
            close_button.pack()

    # Function to play the selected GAE file
    def play_selected():
        # Get the selected GAE file from the entry field
        stop_composition()
        selected_gae_file = gae_file_entry.get()

        # Ensure the selected GAE file is valid
        if selected_gae_file and isfile(selected_gae_file) and selected_gae_file.lower().endswith(".gae"):
            # Load the selected GAE file into the main player with the current generation settings
            seed_str = seed_entry.get()
            generation_pattern = pattern_entry.get()
            composition_thread = threading.Thread(target=generate_composition_in_thread, args=(seed_str, generation_pattern, selected_gae_file))
            composition_thread.start()
    # Create and place a button to play the selected GAE file
    play_selected_button = tk.Button(playlist_manager_window, text="Play Selected", command=play_selected)
    play_selected_button.pack()


# Add a button to open the playlist manager window in the main application window
playlist_manager_button = tk.Button(app, text="Playlist Manager", command=open_playlist_manager_window)
playlist_manager_button.pack()
