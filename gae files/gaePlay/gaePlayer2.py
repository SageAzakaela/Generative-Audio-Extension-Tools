...
def generate_composition_in_thread(seed):
    ...
    for i, layer_folder in enumerate(layer_folders):
        ...
        # Set the random seed to ensure the same shuffling order with the same seed
        seed_for_layer = seed + i if seed is not None else None
        if seed_for_layer is not None:
            random.seed(seed_for_layer)
